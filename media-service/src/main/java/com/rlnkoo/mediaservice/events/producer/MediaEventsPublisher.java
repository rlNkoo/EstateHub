...
public class MediaEventsPublisher {
    ...
    public void publishPhotoUploaded(UUID listingId, PhotoUploadedPayload payload) {
        EventEnvelope<PhotoUploadedPayload> envelope =
                EventEnvelope.of("PhotoUploadedV1", payload);

        eventPublisher.publish(Topics.MEDIA_EVENTS, listingId.toString(), envelope);
    }

    public void publishPhotoDeleted(UUID listingId, PhotoDeletedPayload payload) {
